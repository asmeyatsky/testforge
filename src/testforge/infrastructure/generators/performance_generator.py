"""Performance test generator — produces k6 load test scripts."""
...
class PerformanceGenerator:
    """Generates k6 performance/load test scripts from API endpoints."""
    ...
    def generate(self, strategy: TestStrategy, output_dir: Path) -> TestSuite:
        ...
        if not suite or not suite.test_cases:
            ...
            if not endpoints:
                return TestSuite(layer=TestLayer.PERFORMANCE)
        else:
            endpoints = [
        ...
        template = self._env.get_template("k6_performance.js.j2")
        content = template.render(endpoints=endpoints)
        out_file = output_dir / "performance_test.js"
